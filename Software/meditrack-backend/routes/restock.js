...
router.get('/', (req, res) => {
  ...
  const rows = data.medicines
    .filter((m) => m.patientId === req.user.patientId && m.pillsLeft <= m.threshold)
...
router.post('/:medicineId', (req, res) => {
  const data = readDB();
  const med = data.medicines.find((m) => m.id === +req.params.medicineId && m.patientId === req.user.patientId);
  ...
  data.doseLogs.push({
    ...
    itemName: med.name, action: 'refilled', qty, by: req.user.username, createdAt: nowIso()
  });
