...
function todaysRowsForPatient(data, patientId) {
  const date = todayDate();
  const medicines = data.medicines.filter((m) => m.patientId === patientId);
  ...
  return data.scheduleTimes
    ...
    .map((s) => {
      ...
      return {
        ...
        takenBy: status ? status.takenByName : null,
        takenAt: status ? status.takenAt : null
      };
    })
...
router.get('/today', (req, res) => {
  const data = readDB();
  const rows = todaysRowsForPatient(data, req.user.patientId).map((r) => ({ ...r, state: doseState(r) }));
  res.json(rows);
});
...
router.post('/:scheduleId/taken', (req, res) => {
  ...
  const med = sched && data.medicines.find((m) => m.id === sched.medicineId && m.patientId === req.user.patientId);
  ...
  if (status) {
    ...
    status = {
      ...
      taken: true, takenByName: req.user.username, takenAt: nowStamp
    };
  ...
  data.doseLogs.push({
    id: nextId(data, 'doseLogs'), patientId: req.user.patientId, medicineId: med.id, scheduleId: sched.id,
    itemName: med.name, action: 'taken', qty: 1, by: req.user.username, createdAt: nowStamp
  });
...
router.get('/activity', (req, res) => {
  ...
  const rows = data.doseLogs
    .filter((l) => l.patientId === req.user.patientId)
